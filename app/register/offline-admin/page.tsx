  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 px-6 py-10">
      <section className="rounded-3xl border border-[var(--border)] bg-white p-8 shadow-sm">
        ...
        <p className="mt-3 text-slate-600">Use this temporary fallback when Supabase is unreachable. If you provide LINE info, system will try to bind it as super admin.</p>
        ...
        <form action="/api/auth/offline-admin-login" method="post" className="mt-6 grid gap-4">
          ...
          <label className="grid gap-1 text-sm">
            <span>LINE user ID (optional but recommended)</span>
            <input name="lineUserId" type="text" className="rounded-xl border border-slate-300 px-3 py-2" placeholder="Uxxxxxxxx" />
          </label>
          <label className="grid gap-1 text-sm">
            <span>Full name (TH)</span>
            <input name="fullNameTh" type="text" className="rounded-xl border border-slate-300 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm">
