...
export function RoomManagementPanel() {
  ...
  async function load() {
    ...
    const json = (await response.json()) as { items: AdminRoomRecord[] };
    setItems(json.items);
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('บันทึกห้องเรียนไม่สำเร็จ');
  ...
  return (
    <div className="space-y-6">
      <Card className="space-y-4">
        ...
        <button type="button" onClick={handleSubmit} disabled={submitting} className="inline-flex items-center justify-center rounded-full bg-slate-900 px-4 py-2 text-sm font-medium text-white" style={{ color: '#ffffff' }}>{submitting ? 'กำลังบันทึก...' : 'เพิ่มห้องเรียน'}</button>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {items.map((item) => (
          <Card key={item.roomId}>
            ...
            <div className="mt-3"><Badge tone={tone[item.gpsPolicy]}>{item.gpsPolicy}</Badge></div>
          </Card>
        ))}
      </div>
    </div>
